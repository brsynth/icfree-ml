...
def generate_snakefile(args):
    output_folder = args.plate_design_output_folder
...
rule SAMPLER:
    ...
    output:
        csv="{args.plate_design_sampling_file}"
    params:
...
rule PLATE_DESIGN:
    ...
    params:
        sample_volume={args.plate_design_sample_volume},
        default_dead_volume={args.plate_design_default_dead_volume},
        dead_volumes="{args.plate_design_dead_volumes}",
        num_replicates={args.plate_design_num_replicates},
        well_capacity={args.plate_design_well_capacity},
        start_well_src_plt="{args.plate_design_start_well_src_plt}",
        start_well_dst_plt="{args.plate_design_start_well_dst_plt}",
        output_folder="{args.plate_design_output_folder}"
    shell:
        '''
        python icfree/plate_design.py {{input.sampling_file}} {{params.sample_volume}} \\
...
rule INSTRUCTOR:
    input:
        source_plate=rules.PLATE_DESIGN.output.source_plate,
        destination_plate=rules.PLATE_DESIGN.output.destination_plate
    output:
...
def main():
    ...
    parser.add_argument('--plate_design_sample_volume', default=2000, type=int, help="Sample volume for plate design (default: 2000).")
    parser.add_argument('--plate_design_default_dead_volume', default=20000, type=int, help="Default dead volume (default: 20000).")
    parser.add_argument('--plate_design_dead_volumes', default="RNA=15000,Water=15000", help="Dead volumes for specific components (default: 'RNA=15000,Water=15000').")
    parser.add_argument('--plate_design_num_replicates', default=3, type=int, help="Number of replicates for plate design (default: 3).")
    parser.add_argument('--plate_design_well_capacity', default=50000, type=int, help="Well capacity for plate design (default: 50000).")
    parser.add_argument('--plate_design_start_well_src_plt', default="A1", help="Start well for source plate (default: 'A1').")
    parser.add_argument('--plate_design_start_well_dst_plt', default="A1", help="Start well for destination plate (default: 'A1').")
    parser.add_argument('--plate_design_output_folder', default="output", help="Output folder for plate design (default: 'output').")
    parser.add_argument('--plate_design_sampling_file', required=True, help="Output file for samples generated by the SAMPLER step.")
